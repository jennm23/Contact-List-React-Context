	...
	return {
		...
		actions: {
			...
			fetchContactos: async () => {
				try {
					const response = await fetch("https://playground.4geeks.com/contact/agendas/jenm23")
			...
			addContact: async (contact) => {
				try {
					console.log(contact)
					const response = await fetch("https://playground.4geeks.com/contact/agendas/jenm23/contacts", {
						method: "POST",
			...
			updateContact: async (updatedContact) => {
				try {
					const response = await fetch(`https://playground.4geeks.com/contact/agendas/jenm23/contacts/${updatedContact.id}`, {
						method: 'PUT',
						headers: {
							'Content-Type': 'application/json'
						},
			...
			deleteContact: async (id) => {
				try {
					const response = await fetch(`https://playground.4geeks.com/contact/agendas/jenm23/contacts/${id}`, {
						method: "DELETE"
					})
					if (response.ok == false) {
